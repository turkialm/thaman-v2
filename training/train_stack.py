"""
THAMAN Stacking Model — train_stack.py
=======================================
Adds LightGBM base learner + Ridge meta-learner on top of v2 XGBoost.
Expected: R² 0.544 → ~0.58–0.62

Steps:
  1. Load + engineer features (identical to v2)
  2. Save walk_score MinMaxScaler params to meta.json
  3. OOF predictions: XGBoost + LightGBM (5-fold spatial GroupKFold)
  4. Ridge meta-learner trained on OOF stack
  5. Retrain final XGB + LGB on full work set
  6. Evaluate: XGB alone vs LGB alone vs Stack on holdout
  7. Save: thaman_stack.pkl  +  update meta.json
"""
...
print("=" * 65)
print("  THAMAN Stack v1 — XGBoost + LightGBM + Ridge")
print("=" * 65)
...
print(f"  walk_score scaler params saved for {list(walk_comps.columns)}")
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(X_work, y_work, groups), 1):
    ...
    print(f"  Fold {fold}: XGB R²={r2x:.4f}  LGB R²={r2l:.4f}")
...
# ── Ridge meta-learner ────────────────────────────────────────────
print("\n  Training Ridge meta-learner on OOF stack …")
S_work = np.column_stack([oof_xgb, oof_lgb])
ridge  = Ridge(alpha=1.0)
ridge.fit(S_work, y_work)
oof_stack = ridge.predict(S_work)
print(f"  OOF Stack R²={r2_score(y_work, oof_stack):.4f}  "
      f"MedAPE={medape(np.expm1(y_work), np.expm1(oof_stack)):.2f}%")
print(f"  Ridge weights: XGB={ridge.coef_[0]:.4f}  LGB={ridge.coef_[1]:.4f}  "
      f"intercept={ridge.intercept_:.4f}")

# ── 7. Retrain final models on full work set ──────────────────────
print("\n[7/8] Retraining final XGB + LGB on full work set …")
...
S_hold     = np.column_stack([pred_xgb, pred_lgb])
pred_stack = ridge.predict(S_hold)
...
meta["stack"] = {
    ...
    "ridge": {
        "coef_xgb":      round(float(ridge.coef_[0]), 6),
        "coef_lgb":      round(float(ridge.coef_[1]), 6),
...
# ── Summary ───────────────────────────────────────────────────────
print("\n" + "=" * 65)
print("  THAMAN Stack — Complete")
print("=" * 65)
print(f"\n  {'Model':<12}  {'R²':>7}  {'MAE':>12}  {'MedAPE':>9}")
print(f"  {'-'*46}")
print(f"  {'XGBoost v2':<12}  {r2_xgb:>7.4f}  ${mae_xgb:>11,.0f}  {mape_xgb:>8.2f}%")
print(f"  {'LightGBM':<12}  {r2_lgb:>7.4f}  ${mae_lgb:>11,.0f}  {mape_lgb:>8.2f}%")
print(f"  {'Stack':<12}  {r2_stk:>7.4f}  ${mae_stk:>11,.0f}  {mape_stk:>8.2f}%")
print(f"\n  R² improvement (XGB→Stack): +{r2_stk - r2_xgb:+.4f}")
print(f"  MedAPE improvement        : {mape_xgb:.2f}% → {mape_stk:.2f}%")
print(f"\n  Files saved:")
print(f"    models/thaman_stack.pkl  (LGB + Ridge meta)")
print(f"    models/xgboost_model.json (retrained XGB)")
print(f"    models/meta.json (walk_score_scaler + stack metrics)")
