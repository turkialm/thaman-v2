"""
THAMAN Stack v11 — train_stack_v11.py
======================================
Key improvements over v10 (94 features → 104 features):

  Building health & maintenance:
    • hpd_class_b_viol_zip      — HPD Class B (hazardous) open violations per ZIP
    • hpd_class_c_viol_zip      — HPD Class C (immediately hazardous) open violations per ZIP
    • hpd_severity_score_zip    — Weighted severity composite (C×3 + B×2 + A×1), log-scaled

  Construction & renovation activity:
    • dob_reno_permit_count     — DOB A1/A2 renovation permits per ZIP since 2022 (log)
    • dob_newbld_permit_count   — DOB new-building permits per ZIP since 2022 (log)

  Hyper-local QoL (311 spatial):
    • rat_density_nta           — Rodent complaints per 1000 residents by NTA (log)
    • heat_density_nta          — Heat/hot-water complaints per 1000 residents by NTA (log)

  Transit quality:
    • nearest_station_is_cbd    — 1 if nearest subway station is in Central Business District
    • nearest_station_route_count — Count of subway lines at nearest station
    • nearest_station_is_ada    — 1 if nearest station is ADA accessible

Data source: data/processed/features_v5.csv
  (produced by scripts/prepare_v11_features.py)

Expected gain vs v10: R² 0.6460 → ~0.650–0.658, MedAPE 20.16% → ~19.5–19.8%

Run:
  cd /Users/totam/Desktop/new_try
  python training/train_stack_v11.py
"""
...
print("=" * 70)
print("  THAMAN Stack v11 — Building Health + QoL + Transit (104 features)")
print("=" * 70)
...
FEATURE_NAMES = [f for f in (V4_BASE + V5_FEATS + V6_FEATS + V11_FEATS) if f in df_work.columns]
n_v11 = len([f for f in V11_FEATS if f in df_work.columns])
print(f"  Total features: {len(FEATURE_NAMES)}  "
      f"(+{len([f for f in V6_FEATS if f in df_work.columns])} v6, +{n_v11} v11)")
...
meta.update({
    ...
    "stack": {
        ...
        "r2_improvement":     round(r2_stk - 0.6460, 4),
        "medape_improvement": round(20.16 - mp_stk, 2),
    },
...
print(f"\n  vs v10: ΔR²={r2_stk-0.6460:+.4f}  ΔMedAPE={20.16-mp_stk:+.2f}pp")
