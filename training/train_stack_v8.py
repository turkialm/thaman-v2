"""
THAMAN Stack v8 — train_stack_v8.py
=====================================
Fixes v7 regression. v7 hit R²=0.633 (worse than v6's 0.645) because:
  - Bayesian smoothing k=30 shrunk bldgclass_encoded (top SHAP feature) by
    20–37%, destroying its signal
  - 3 interaction features (nta_rel_price, sqft_x_nta_enc, bldg_age_x_nta)
    had ranges up to ~700, adding noise without signal

v8 keeps all genuinely useful v7 additions and reverts the harmful ones:

  ✓ KEPT   — ZIP code encoding (zip_encoded, zip_bldg_encoded)
  ✓ KEPT   — NTA price trend (nta_price_trend)
  ✓ KEPT   — 10-fold GroupKFold
  ✓ KEPT   — 5 000 estimators + patience=400
  ✗ REMOVED — Bayesian smoothing → raw mean encoding (same as v6, proven best)
  ✗ REMOVED — nta_rel_price, sqft_x_nta_enc, bldg_age_x_nta interactions

Net new features vs v6: zip_encoded, zip_bldg_encoded, nta_price_trend (+3)
Total: 96 features (v6: 93)

Expected vs v6: R² 0.645 → ~0.65–0.67, MedAPE 20.19% → ~19–20%

Run:
  cd /Users/totam/Desktop/new_try
  python training/train_stack_v8.py
"""
...
print("=" * 70)
print("  THAMAN Stack v8 — ZIP + NTA Trend + Raw Encoding")
print("=" * 70)
...
SMOOTH_K = 0    # v8: raw mean encoding (Bayesian k=30 degraded bldgclass signal in v7)
...
def _smooth_encode_pl(df_src, df_apply, key_col, target_col, alias, gm, k=SMOOTH_K):
    """Polars groupby → raw mean encoding → join. k=0 means no smoothing."""
    stats = (df_src.group_by(key_col)
             .agg(pl.col(target_col).mean().alias(alias))
...
# v8: interaction features removed (caused noise in v7, ranges up to ~700)
v8_interact = []
...
V8_FEATS = zip_features + v8_interact
FEATURE_NAMES = [f for f in (V4_BASE + V5_FEATS + V6_FEATS + V8_FEATS) if f in df_work.columns]
print(f"  Total features: {len(FEATURE_NAMES)}  "
      f"(v6: {len([f for f in V6_FEATS if f in df_work.columns])}  "
      f"v8new: {len([f for f in V8_FEATS if f in df_work.columns])})")
...
meta.update({
    ...
    "stack": {
        ...
        "r2_improvement":     round(r2_stk - 0.6331, 4),
        "medape_improvement": round(21.45 - mp_stk, 2),
    },
})
...
print("\n" + "="*70)
print("  THAMAN Stack v8 — Complete")
print("="*70)
...
for nm,r,m,mp_ in [("XGB-A",r2_xa,mae_xa,mp_xa),("XGB-B",r2_xb,mae_xb,mp_xb),
                   ...
                   ("Stack v8",r2_stk,mae_stk,mp_stk)]:
    print(f"  {nm:<24}  {r:>7.4f}  ${m:>13,.0f}  {mp_:>8.2f}%")
print(f"\n  vs v6: ΔR²={r2_stk-0.6454:+.4f}  ΔMedAPE={20.19-mp_stk:+.2f}pp")
print(f"  Total features: {len(FEATURE_NAMES)}  (v7 new: {len([f for f in V8_FEATS if f in df_work.columns])})")
