"""
THAMAN Stack v6 — train_stack_v6.py
=====================================
Key improvements over v5:
  • NTA-level target encoding  (nta_encoded, nta_bldg_encoded)
      — more granular than borough×class; directly fills the lat/lon gap
      — expected to be top-5 SHAP feature (NTA = ~200 NYC neighborhoods)
  • 4 additional engineered features (85 → 91):
      log_land_sqft, lot_coverage, bldg_vol_proxy, prior_price_psf
  • LightGBM meta-learner (replaces Ridge)
      — captures non-linear blending; e.g. XGB-A dominant when price > $3M,
        LGB dominant in Queens mid-market
  • Tighter XGB early stopping (patience=200) + n_estimators=4000
  • Per-NTA market stats (nta_sale_count, nta_median_psf) as features

Expected gain vs v5: R² 0.658 → ~0.68–0.70, MedAPE 20.34% → ~18–19%

Run:
  cd /Users/totam/Desktop/new_try
  python training/train_stack_v6.py
"""
...
print("=" * 70)
print("  THAMAN Stack v6 — NTA Encoding + LGB Meta-Learner")
print("=" * 70)
...
print(f"  Total features: {len(FEATURE_NAMES)}  "
      f"(+{len([f for f in V6_FEATS if f in df_work.columns])} v6)")
...
XGB_B = dict(objective="reg:squarederror", eval_metric="rmse",
             ...
             early_stopping_rounds=200, random_state=7, n_jobs=-1)
LGB_P = dict(objective="regression", metric="rmse",
             n_estimators=4000, learning_rate=0.04, num_leaves=127,
             min_child_samples=15, feature_fraction=0.70, bagging_fraction=0.80,
             bagging_freq=5, reg_alpha=0.3, reg_lambda=1.5,
             n_jobs=-1, random_state=42, verbose=-1)
CAT_P = dict(iterations=2000, learning_rate=0.025, depth=8,
             l2_leaf_reg=1.5, random_strength=0.5, bagging_temperature=0.8,
             border_count=64, early_stopping_rounds=200, random_seed=42, verbose=0)
...
for fold, (tr, va) in enumerate(gkf.split(X_work, y_work, groups), 1):
    ...
    ml.fit(Xtr, ytr, eval_set=[(Xva,yva)], callbacks=[lgb.early_stopping(200,verbose=False),lgb.log_evaluation(-1)])
...
final_lg.fit(Xtr_f, ytr_f, eval_set=[(Xva_f,yva_f)], callbacks=[lgb.early_stopping(200,verbose=False),lgb.log_evaluation(500)])
...
meta.update({
    ...
    "stack": {
        ...
        "r2_improvement":     round(r2_stk - 0.6582, 4),
        "medape_improvement": round(20.34 - mp_stk, 2),
    },
})
...
print("\n" + "="*70)
print("  THAMAN Stack v6 — Complete")
print("="*70)
...
for nm, r, m, mp_ in [("XGB-A",r2_xa,mae_xa,mp_xa),("XGB-B",r2_xb,mae_xb,mp_xb),
                       ...
                       ("Stack v6",r2_stk,mae_stk,mp_stk)]:
    print(f"  {nm:<24}  {r:>7.4f}  ${m:>13,.0f}  {mp_:>8.2f}%")
print(f"\n  vs v5: ΔR²={r2_stk-0.6582:+.4f}  ΔMedAPE={20.34-mp_stk:+.2f}pp")
