...
\begin{question}{Why is Riyadh holdout $R^2=0.80$ lower than OOF $R^2=0.93$?}
\begin{shortanswer}
...
\begin{itemize}
  ...
  \item A gap of $\approx 13\,R^2$ points ($0.93 \to 0.80$) is expected and
        honest. The holdout metric is the number to cite: $R^2=0.80$,
        MedAPE$=15.59\%$.
\end{itemize}
\end{detail}
\end{question}
...
\begin{question}{Your NYC model has $R^2=0.65$ but Riyadh has $R^2=0.80$.
...
\begin{question}{MedAPE of 15.59\% seems high. Is that good enough?}
\begin{shortanswer}
Industry benchmark for AVM in emerging markets is 15--25\%. 15.59\% is within
...
\begin{itemize}
  ...
  \item Given the data constraint, 15.59\% on 2025 holdout (new macro period)
        is competitive. OOF MedAPE = 8.25\% shows the model fits well
        within-sample.
  \item By property type: apartment MedAPE = 12.83\%, villa = 12.39\%.
        Residential plot has highest error (20.82\%) because land prices are
...
\begin{itemize}
  ...
  \item For 2026 predictions: acknowledge the policy-shift caveat.
        The 15.59\% holdout MedAPE is expected to widen slightly until
...
8.25\% vs.\ 15.59\%), confirming that adding 2024 Metro-era data was the right
call.
\end{detail}
\end{question}
...
\end{enumerate}
If pressed on the plots segment specifically: error analysis shows plot MedAPE
(20.8\%) is systematic underprediction --- median signed error $-15\%$, reaching
$-20\%$ in 2025~Q1 --- because plot prices surged in 2025 faster than the lagged
district features could track, especially in illiquid districts (3--5 deeds per
quarter: 33\% MedAPE). Citywide per-type lag and momentum features were
empirically tested and did not improve the plot segment; the fix is a dedicated
plots model or post-hoc bias calibration, documented as future work.
\end{detail}
\end{question}
...
\begin{enumerate}
  ...
  \item \textbf{Riyadh model trained through 2024; predictions for 2025--2026
        ...
        data ($R^2=0.80$) validates it remains useful one year forward.''}
