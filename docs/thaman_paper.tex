...
6,910 district-level real estate transactions (2018--2025). Across 149 features
...
market signals (SA\_Aqar), the Riyadh stack achieves OOF $R^2 = 0.9348$ /
MedAPE~$= 8.25\%$ and holdout $R^2 = 0.8014$ / MedAPE~$= 15.59\%$ on a 2025
Q1--Q3 out-of-sample test set of 1,730 district-quarter observations.
...
\begin{itemize}
  ...
  \item A bilingual (English/Arabic) interactive valuation interface with SHAP
        ...
        choropleth.
  \item Riyadh Stack v12: a dual-city extension to Saudi Arabia using 6,910
        district-quarter observations, 149 features, and the identical stacking
        architecture. Achieves holdout $R^2 = 0.8014$ / MedAPE $= 15.59\%$ on
        a fully out-of-sample 2025 Q1--Q3 test set (1,730 rows).
\end{itemize}
...
End-to-end prediction latency: 200--400\,ms locally; $\sim$0.7--1.2\,s on the deployed Space (free-tier CPU + network)
\begin{description}
...
Q1--Q3 2025 (1,730 rows).

\subsection{Feature Engineering (149 features)}
...
\begin{table}[H]
  ...
  \begin{tabular}{lrr}
    ...
    Holdout (Q1--Q3 2025, $n=1{,}730$) & 0.8014 & 15.59\% \\
    \bottomrule
  \end{tabular}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.5cm}p{9cm}}
    ...
    1.\ Credibility \& accuracy &
      MedAPE$=20.32\%$ on 27,763 holdout transactions (NYC); MedAPE$=15.59\%$
      on 1,730 out-of-sample observations (Riyadh). Borough- and
    ...
    4.\ Random sample testing &
      ...
      Q1--Q3 2025 holdout ($n=1{,}730$, Riyadh). Spatial GroupKFold CV
...
\begin{enumerate}
  ...
  \item \textbf{Riyadh granularity}: district-level aggregates cannot capture within-district
        variance.
  \item \textbf{Residential-plot temporal shift}: plots are the weakest Riyadh segment
        (holdout MedAPE 20.82\% vs 12.4\% for villas). Error analysis shows this is
        systematic underprediction (median signed error $-15\%$, reaching $-20\%$ in
        2025~Q1): plot prices surged in 2025 faster than the lagged district features
        could track. The effect is amplified in illiquid districts (3--5 deeds per
        quarter: 33\% MedAPE) and the top price quartile (28\%). Adding citywide
        per-type lag and momentum features was tested and did not improve the plot
        segment, indicating the shift is not recoverable from lag features alone.
...
\begin{enumerate}
  ...
  \item Riyadh parcel-level data: as Ministry of Justice individual deed data becomes
        accessible.
  \item Residential-plot segment model: either a dedicated plots model or per-type
        post-hoc bias calibration against recent quarters, targeting the documented
        $-15\%$ underprediction under the 2025 price surge.
...
observations (2018--2025) across 149 features, the Riyadh stack achieves OOF
$R^2 = 0.9348$ / MedAPE $= 8.25\%$ and holdout $R^2 = 0.8014$ / MedAPE
$= 15.59\%$ on a fully out-of-sample 2025 Q1--Q3 horizon (1,730 rows). The identical
